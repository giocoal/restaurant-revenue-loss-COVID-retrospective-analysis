...
f_mod3 <- function(Y, h, newxreg = NULL) {
  ...
  for (i in 1:h) {
    ...
    X[1, 2:14] <- X[1, 1:13]
    # Aggiorniamo anche i regressori
    X[1, 15:ncol(X)] <- newxreg[i, 15:ncol(newxreg)]
    X[1, 1] <- y_hat[i]
  }
  return(y_hat)
}
...
e1 <- tsCV_RandomForest(my_xts = rf_r1[,1], forecastfunction = f_mod1, h=h, initial = initial, window = window)
...
e2 <- tsCV_RandomForest(my_xts = data[,1], xreg = data[,-1], forecastfunction = f_mod2, h=h, initial = initial, window = window)
...
e3 <- tsCV_RandomForest(my_xts = data3[,1], xreg = data3[,-1], forecastfunction = f_mod3, h=h, initial = initial, window = window)
```
