...
fest <- read_xlsx("C:\\Users\\gianl\\Desktop\\Uni Bicocca\\GitHub\\Data-Science-Lab\\Dati aggiuntivi\\fest.xlsx")
...
## Modello 5 (usando regressori festivita)
...
# prova <- merge(x=precovid1[, c(8)], y=fest[,-1], by.x=precovid1$data, by.y=fest$date)
...
```{r}
mod5 <- SSModel(log(lordototale)~dec24+dec25+dec26+jan1+jan6+aug15+dec31+eastsun+eastermon+apr25+mag1+jun2+oct31+nov1+martgrasso+#bridge+#Giorno_Monday+Giorno_Tuesday+Giorno_Wednesday+Giorno_Thursday+Giorno_Saturday+Giorno_Sunday+
                  SSMtrend(2, list(NA, NA))+
                  SSMseasonal(7, NA, 'trigonometric'),
                H=NA,
                data=train)
