...
```{r}
plot(log(val$lordototale[validation_date:nrow(val),]), lwd=3) # serie storica
lines(muhat1_pre[validation_date:nrow(val),], type = "l", col = "red", lwd=3) # serie predetta [1801:1826,]
```

```{r}
mape(val$lordototale[validation_date:nrow(val),], exp(muhat1_pre[validation_date:nrow(val),]))
rmse(val$lordototale[validation_date:nrow(val),], exp(muhat1_pre[validation_date:nrow(val),]))
```

```{r}
plot(val$lordototale - exp(muhat1_pre))
```
...
```{r}
mape(val$lordototale[validation_date:nrow(val),], exp(muhat4_pre[validation_date:nrow(val),]))
rmse(val$lordototale[validation_date:nrow(val),], exp(muhat4_pre[validation_date:nrow(val),]))
```
...
```{r}
plot(val$lordototale[validation_date:nrow(val),], lwd=2, ylim=c(9000,40000))
lines(exp(muhat5_pre[validation_date:nrow(val),]), type = "l", col = "red", lwd=3) # [1801:1826,]
```

```{r}
mape(val$lordototale[validation_date:nrow(val),], exp(muhat5_pre[validation_date:nrow(val),]))
rmse(val$lordototale[validation_date:nrow(val),], exp(muhat5_pre[validation_date:nrow(val),]))
```

```{r}
summary(val$lordototale - exp(muhat5_pre))
```

```{r}
plot(val$lordototale - exp(muhat5_pre))
...
comp <- data.frame(Modello=c("mod1", "mod4", "mod5"),
           RMSE=c(rmse(val$lordototale[validation_date:nrow(val),], exp(muhat1_pre[validation_date:nrow(val),])),
                  rmse(val$lordototale[validation_date:nrow(val),], exp(muhat4_pre[validation_date:nrow(val),])),
                  rmse(val$lordototale[validation_date:nrow(val),], exp(muhat5_pre[validation_date:nrow(val),]))),
           MAPE=c(mape(val$lordototale[validation_date:nrow(val),], exp(muhat1_pre[validation_date:nrow(val),])),
                  mape(val$lordototale[validation_date:nrow(val),], exp(muhat4_pre[validation_date:nrow(val),])),
                  mape(val$lordototale[validation_date:nrow(val),], exp(muhat5_pre[validation_date:nrow(val),]))))
comp
```
...
lines(exp(muhat1_pre[validation_date:nrow(val),]), type = "l", col = "red", lwd=3) # [1801:1826,]
lines(exp(muhat4_pre[validation_date:nrow(val),]), type = "l", col = "blue", lwd=3) # [1801:1826,]
lines(exp(muhat5_pre[validation_date:nrow(val),]), type = "l", col = "green", lwd=3) # [1801:1826,]
```
...
# Zoom out
plot(1:42, RMSE_mod1, type="l", col=1, xlab="horizon", ylab="RMSE", ylim = c(0,10000))
lines(1:42, RMSE_mod4, type="l",col=2)
lines(1:42, RMSE_mod5, type="l",col=3)
legend("topleft",legend=c("1_UCM","4_UCM","5_UCM"),col=1:4,lty=1)
abline(h=5000)
...
lines(1:42, MAE_mod4, type="l",col=2)
lines(1:42, MAE_mod5, type="l",col=3)
...
plot(1:42, MAPE_mod1, type="l", col=1, xlab="horizon", ylab="MAPE", ylim = c(0,13000))
...
```{r}
plot(log(val$lordototale[validation_date:nrow(val),] + 10000), lwd=3) # serie storica
lines(muhat1_post[validation_date:nrow(val),], type = "l", col = "red", lwd=3) # serie predetta [1801:1826,]
```

```{r}
mape(val$lordototale[validation_date:nrow(val),], exp(muhat1_post[validation_date:nrow(val),]) - 10000)
rmse(val$lordototale[validation_date:nrow(val),], exp(muhat1_post[validation_date:nrow(val),]) - 10000)
```

```{r}
plot(val$lordototale - (exp(muhat1_post) - 10000))
```
...
```{r}
plot(log(val$lordototale[validation_date:nrow(val),] + 10000), lwd=3) # serie storica
lines(muhat3_post[validation_date:nrow(val),], type = "l", col = "red", lwd=3) # serie predetta
```

```{r}
mape(val$lordototale[validation_date:nrow(val),], exp(muhat3_post[validation_date:nrow(val),]) - 10000)
rmse(val$lordototale[validation_date:nrow(val),], exp(muhat3_post[validation_date:nrow(val),]) - 10000)
```
...
```{r}
mape(val$lordototale[validation_date:nrow(val),], exp(muhat4_post[validation_date:nrow(val),]) - 10000)
rmse(val$lordototale[validation_date:nrow(val),], exp(muhat4_post[validation_date:nrow(val),]) - 10000)
```
...
lines(exp(muhat5_post[validation_date:nrow(val),]) - 10000, type = "l", col = "red", lwd=3) # [1801:1826,]
```

```{r}
mape(val$lordototale[validation_date:nrow(val),], exp(muhat5_post[validation_date:nrow(val),]) - 10000)
rmse(val$lordototale[validation_date:nrow(val),], exp(muhat5_post[validation_date:nrow(val),]) - 10000)
```

```{r}
summary(val$lordototale - (exp(muhat5_post) - 10000))
```

```{r}
plot(val$lordototale - (exp(muhat5_post)) - 10000)
abline(v=index(val[validation_date])) # non vine plottata
```
...
comp <- data.frame(Modello=c("mod1", "mod3", "mod4", "mod5"),
           RMSE=c(rmse(val$lordototale[validation_date:nrow(val),], exp(muhat1_post[validation_date:nrow(val),]) - 10000),
                  rmse(val$lordototale[validation_date:nrow(val),], exp(muhat3_post[validation_date:nrow(val),]) - 10000),
                  rmse(val$lordototale[validation_date:nrow(val),], exp(muhat4_post[validation_date:nrow(val),]) - 10000),
                  rmse(val$lordototale[validation_date:nrow(val),], exp(muhat5_post[validation_date:nrow(val),]) - 10000)),
           MAPE=c(mape(val$lordototale[validation_date:nrow(val),], exp(muhat1_post[validation_date:nrow(val),]) - 10000),
                  mape(val$lordototale[validation_date:nrow(val),], exp(muhat3_post[validation_date:nrow(val),]) - 10000),
                  mape(val$lordototale[validation_date:nrow(val),], exp(muhat4_post[validation_date:nrow(val),]) - 10000),
                  mape(val$lordototale[validation_date:nrow(val),], exp(muhat5_post[validation_date:nrow(val),]) - 10000)))
comp
...
lines(exp(muhat1_post[validation_date:nrow(val),]) - 10000, type = "l", col = "red", lwd=3) # [1801:1826,]
lines(exp(muhat4_post[validation_date:nrow(val),]) - 10000, type = "l", col = "blue", lwd=3) # [1801:1826,]
lines(exp(muhat5_post[validation_date:nrow(val),]) - 10000, type = "l", col = "green", lwd=3) # [1801:1826,]
```
