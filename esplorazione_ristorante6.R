...
r6_pre_covid_rf <- copy_ristorante6_pre_covid[, c('lordototale', 'data', 'Giorno', 
                                                  'Month', 'Season', 'Weekend',
                                                  'Festivo', 'Pioggia')]

# divisione in train e test
index_rf <- sample(1:nrow(r6_pre_covid_rf),
                   size = 0.7*nrow(r6_pre_covid_rf))
train_rf <- r6_pre_covid_rf[index_rf,]
test_rf <- r6_pre_covid_rf[-index_rf,] 
...
copy_ristorante6_rf <- copy_ristorante6[, c('lordototale', 'data', 'Giorno', 'Month', 
...
ristorante6_periodo_covid <- copy_ristorante6_rf %>%
  filter(copy_ristorante6_rf$data >= reference_date_rf)
...
# vendite/scontrini
ristorante6_periodo_covid <- ristorante6_periodo_covid[1:133,]

# si utilizza il modello appena creato per fare previsioni
vendite_forecast_rf <- predict(MRF, ristorante6_periodo_covid)
...
# serie storica dati reali fino a prima covid (r6_pre_covid_rf$lordototale)
r6_pre_covid_rf <- head(r6_pre_covid_rf, -1) # Andava tolta l'ultima riga
...
interval_pre_covid_df <- data.frame(date = interval_pre_covid, 
                                    val=r6_pre_covid_rf$lordototale)
...
ristorante6_complete <- copy_ristorante6[1:625,]  # fino al 17 maggio 2020
ristorante6_complete$lordototale[is.na(ristorante6_complete$vendite6)] <- 0
...
lines(rf_complete$data, rf_complete$`ristorante6_complete$lordototale`, col="red",lty=2)
...
ristorante6_future$Year <- year(ristorante6_future$data)
ristorante6_future$Year <- factor(ristorante6_future$Year, levels = c("2018", "2019", "2020", "2021", "2022"))
...
# colonna ColoreCOVID
ristorante6_future$ColoreCOVID <- "bianco"
ristorante6_future$ColoreCOVID <- factor(ristorante6_future$ColoreCOVID, levels = c("", "arancione", "bianco", "giallo", "rosso"))
...
ristorante6_future <- data.frame(ristorante6_future$data,
                                 ...
                                 ristorante6_future$Year,
                                 ristorante6_future$Weekend,
                                 ristorante6_future$ColoreCOVID)
...
ristorante6_RF_full <-rbind(copy_ristorante6[,c("data", "lordototale", "Giorno",
                                                "Month", "Year", "Weekend", 
                                                "ColoreCOVID")],
                            ristorante6_future)
...
MRF_future_best <- randomForest(lordototale ~ Giorno + Month + Year + Weekend
                                + ColoreCOVID, data = ristorante6_RF_full[1:1337,]) # faccio cos? per non avere problemi sui
# livelli dei factor
...
vendite_forecast_rf_new <- predict(MRF_future_best, ristorante6_RF_full[1339:1491,])
...
vendite_pre_aprile <- copy_ristorante6 %>%
...
RMSE.rf <- sqrt(mean((MRF_future_best$predicted - copy_ristorante6$lordototale)^2))  # 1537.093
